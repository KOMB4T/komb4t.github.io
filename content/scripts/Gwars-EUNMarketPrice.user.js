...
;(function () {
  ...
  function enhancePriceColumns(attempt = 0) {
    ...
    const priceCells = Array.from(document.querySelectorAll("tr > td:first-child")).filter((cell) =>
    ...
    const marketPrice =
      marketPriceData && Number.isFinite(marketPriceData.price) ? marketPriceData.price : null
    ...
    priceCells.forEach((cell) => {
      ...
      const cleanPrice = priceValue / basePrice
      const dirtyPrice = priceValue / governmentPrice
      ...
      cleanSpan.title = `${Math.round(cleanPrice).toLocaleString("ru-RU")} за ${basePrice.toLocaleString("ru-RU")} EUN`
      ...
      dirtySpan.title = `${dirtyEunValue.toLocaleString("ru-RU")} за ${governmentPrice.toLocaleString("ru-RU")} EUN`
  ...
  function extractBankPrice(doc) {
    const link = Array.from(doc.querySelectorAll("a")).find((a) =>
      /Обмен\s+Гб\s+на\s+EUN/i.test(a.textContent),
    )
    if (!link) return null
  ...
  function extractCommercialStats(doc) {
    const rows = Array.from(doc.querySelectorAll("tr")).filter(
      (tr) => tr.cells && tr.cells.length >= 5 && /\$/.test(tr.cells[0].textContent),
    )
  ...
  async function fetchMarketPrice(force = false) {
    ...
    try {
      ...
      console.log(
        "[EUNMarketPrice] Рыночный курс обновлён:",
        minPrice,
        "ГБ/EUN",
        "Мин. комм.:",
        commercialMin ?? "н/д",
